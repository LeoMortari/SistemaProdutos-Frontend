...
const teste = async () => {
  ...
  let td = `<tr>
   ...
   <td id="${produtoVenda[0].valor - total}">${
    produtoVenda[0].valor - total
  }</td>
   </tr>`;
  document.getElementById("tabela").innerHTML = td;

  let bot =
    '<button class="btn btn-success" type="submit" onclick="adicionahandlefinan()"> Enviar</button>';
...
async function teste2() {
  ...
  result.map((lista) => {
    ...
    cell4.innerHTML =
      "R$" + String(lista.ValorLucro.toFixed(2)).replace(".", ",");
    cell2.innerHTML =
      "R$" + String(lista.ValorVenda.toFixed(2)).replace(".", ",");
    cell3.innerHTML =
      "R$" + String(lista.ValorPedido.toFixed(2)).replace(".", ",");
